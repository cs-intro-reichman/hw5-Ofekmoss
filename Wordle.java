public class Wordle {
    ...
    public static boolean containsChar(String secret, char c) {
        return secret.chars().anyMatch(letter -> c == letter);
    }
    ...
    public static void computeFeedback(String secret, String guess, char[] resultRow) {
        for (int i=0; i< secret.length(); i++) {
            resultRow[i] = secret.charAt(i) == guess.charAt(i) ? 'G' : containsChar(secret, guess.charAt(i)) ? 'Y' : '_';
        }
    }
    ...
    public static void main(String[] args) {
        ...
        while (attempt < MAX_ATTEMPTS && !won) {
            ...
            char[] guessFeedback = new char[WORD_LENGTH];
            storeGuess(guess, guesses, attempt);
            computeFeedback(secret, guess, guessFeedback);
            results[attempt] = guessFeedback;
